"""Simulator-reference neural ratio estimation and persistent numerical banks.

Artifacts live below the caller's chosen directory. Completed banks and
ensemble members are immutable, content-checked caches. Only NumPy is
imported eagerly; PyTorch is loaded when training or evaluating networks.
"""
...
SIMULATOR_VERSION = "nre-asimov-selected-simulator-v1"
TRAINING_VERSION = "nre-asimov-bce-swish-nadam-v2"
BANK_VERSION = "nre-asimov-immutable-bank-v1"
...
def _verified_manifest(directory, contract):
    ...
    try:
        ...
        raise CacheError(
            ...
            "Inspect or move only this artifact aside before rerunning; "
            "completed training has not been changed."
        ) from error
    return payload
...
def simulate_reconstructed(component, n, rng):
    """Generate reconstructed events from the Gaussian-mixture detector model."""
    from utils_distributions import background_components, signal_components, smearing_parameters
...
def train_nre(root, train_signal, train_background, train_ref,
    ...
    Defaults use four independently initialized 4x1024 SiLU networks per
