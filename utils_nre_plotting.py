"""Inline plotting helpers for the NRE Asimov demonstration.

These helpers consume numerical arrays and return Matplotlib figures without
writing files.  Figures remain open for display with ``plt.show()`` in the
notebook; ``plot_training`` returns a dictionary of figures.
"""
...
def plot_training(histories):
    """Plot ensemble losses and, when recorded, the learning-rate history."""
    figures = {}
    with plt.rc_context(_STYLE):
        ...
        # Histories without recorded learning rates remain plottable. Otherwise,
        # show the rate used for every epoch and the deployed checkpoint.
...
def plot_simulator_score_closure(diagnostics, summary):
    ...
    with plt.rc_context(_STYLE):
        ...
        ax.errorbar(positions, scores, yerr=errors, fmt="o", color="C0", capsize=3,
                    label="Independent banks (MC error)")
        ax.errorbar([len(scores) + 1], [summary["score_mean"]],
...
def plot_ratio_validation(ref_ratios, signal_ratios, background_ratios):
...
def plot_mle_convergence(study):
    ...
    with plt.rc_context(_STYLE):
        ...
        for key, color, label, marker in (
            ...
            ax.errorbar(sizes, estimates.mean(axis=0), yerr=spread, color=color,
...
def plot_asimov_scans(results, sizes, label):
...
def _toy_histogram(ax, values, bins, label, color, markers=False):
    ...
    if markers:
        centers = 0.5 * (bins[:-1] + bins[1:])
        ax.errorbar(centers, probability, yerr=np.sqrt(counts) / values.size,
...
def plot_compression_validation(validation):
